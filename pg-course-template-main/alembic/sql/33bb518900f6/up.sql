...
-- 2. Создаем новые роли для авторизации в приложении
CREATE ROLE inventory_manager WITH LOGIN PASSWORD 'pass';
CREATE ROLE worker WITH LOGIN PASSWORD 'pass';

-- 3. Инициализируем новую схему для инвентаря
CREATE SCHEMA IF NOT EXISTS inventory;
-- 1. СТРУКТУРА ТАБЛИЦ В СХЕМЕ CATALOG (Справочники)
-- =====================================================================

-- Таблица маршрутов перемещений с типом INTERVAL и DECIMAL
...
-- =====================================================================
-- 2. СТРУКТУРА ТАБЛИЦ В СХЕМЕ INVENTORY (Бизнес-процессы инвентаря)
-- =====================================================================

-- Таблица остатков и резервов (Составной PK вместо SERIAL id)
...
-- Накладные на доставку клиентам
...
-- Позиции накладных на доставку (Слабая сущность, составной PK)
...
-- Накладные на межскладское перемещение
...
-- Позиции накладных на перемещение (Слабая сущность, составной PK)
...
-- =====================================================================
-- 3. НАСТРОЙКА ПРАВ ДОСТУПА И БЕЗОПАСНОСТИ ДЛЯ РОЛЕЙ
-- =====================================================================

-- Права для INVENTORY_MANAGER (Полный доступ к inventory, чтение sales/catalog)
...
-- Права для WORKER (Только инвентарный контур без доступа к продажам)
